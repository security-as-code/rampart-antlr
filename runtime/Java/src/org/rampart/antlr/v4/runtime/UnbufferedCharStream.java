...
public class UnbufferedCharStream implements CharStream {
	...
	public UnbufferedCharStream(InputStream input, int bufferSize) {
		this(input, bufferSize, "UTF-8");
	}

	public UnbufferedCharStream(InputStream input, int bufferSize, String charsetName) {
		this(bufferSize);
		try {
			this.input = new InputStreamReader(input, charsetName);
